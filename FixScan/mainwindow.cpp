    ...
    ui(new Ui::MainWindow)
{
	...
	ui->comboBox__AlignType->setVisible(false);
	ui->label_7->setVisible(false);
	ui->lineEdit_PointDistance->setVisible(false);
	ui->pushButton_PauseTurnable->setDisabled(true);
	ui->pushButton_scanSimplify->setVisible(false);
	...
	QTimer::singleShot(0, this, [&]{
		...
		foreach(QString key ,list)
		{
			if (key == "cali")
			{
				QJsonObject objs = obj[key].toObject();
				QStringList list = objs.keys();
				if (!objs["exist"].toBool())
				{
					//m_heartbeatTimer->stop();

 					QMessageBox::critical(this, "Error", "Cali File Error!");
...
void MainWindow::on_pushButton_NewProject_clicked()
{
	...
	if (strPath == "")
	{
		QMessageBox::warning(this, tr("Error"), tr("File Path Is Null!"));
		return;
	}
...
void MainWindow::brightnessChanged()
{
	...
	int nbytes = zmq_send(m_zmqReqSocket, sendData, strlen(sendData), ZMQ_SNDMORE);
	if (nbytes != strlen(sendData)) {
		qWarning() << "cannot send brightness ZMQ_SNDMORE" << "nbytes" << nbytes;
		return;
	}
...
void MainWindow::on_pushButton_ScanNewProFilePath_clicked()
{
	...
	QString path = QFileDialog::getSaveFileName(this, QStringLiteral("save a file"));
	ui->lineEdit_NewProFilePath->setText(path);
}

void MainWindow::on_pushButton_GlobalMarker_clicked()
{
	qDebug() << "Select A Global Marker File";
	m_progressDialog->setWindowTitle("Global Marker File");
	QString path = QFileDialog::getOpenFileName(this, QStringLiteral("select a file"),"","P3(*.p3)");
	ui->lineEdit_GlobalMarkerFileR->setText(path);
}
...
void MainWindow::on_pushButton_ScaneEnterScan_clicked()
{
    ...
    on_pushButton_RegisterProcesser_clicked();// 进入扫描就打开视频流
}
...
{
	QString str = QFileDialog::getOpenFileName(this,QStringLiteral("select a file"),"/home","sln(*.sln_fix)");
	...
	nbytes = zmq_send(m_zmqReqSocket, str.toLocal8Bit(), str.toLocal8Bit().size(), 0);
	if (nbytes != str.toLocal8Bit().size()) {
...
void MainWindow::onPublishReceived(QString majorCmd, QString minorCmd, QByteArray data)
{
	if (majorCmd == QStringLiteral("beginAsyncAction")) {
		...
		ui->pushButton_PauseTurnable->setDisabled(false);
...
void MainWindow::onVideoImageReady(int camID, QPixmap pixmap)
{
	...
	if (camID == 0){
		...
		ui->label_Cam0->setPixmap(pixmap.transformed(matrix, Qt::SmoothTransformation));
	}
	else if (camID == 1){
